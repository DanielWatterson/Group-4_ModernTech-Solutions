      ...
      <div class="col-12 mt-3 mb-4">
        <div class="card">
          <div class="card-header bg-secondary text-white fw-bold">
            Weekly Attendance Performance (Latest Mock Week)
          </div>
          <div class="table-responsive">
...
// --- Initial Mock Data for Balances and Attendance ---

// 1. Initial Leave Balances for ALL employees
const mockLeaveBalances = employeeInfoJSON.employeeInformation.map(emp => ({
    ...
    sickBalance: 10,
    familyBalance: 5,
    unpaidBalance: 999, // Unpaid leave is tracked but not deducted from a typical "limit"
}));

// 2. Mock Weekly Attendance Data
const mockWeeklyAttendance = [
    { employeeId: 1, week: 'Dec Week 1', daysPresent: 5, daysLate: 0, performance: 'Excellent' },
    { employeeId: 2, week: 'Dec Week 1', daysPresent: 4, daysLate: 0, performance: 'Good' },
    { employeeId: 3, week: 'Dec Week 1', daysPresent: 3, daysLate: 2, performance: 'Needs Improvement' },
    { employeeId: 4, week: 'Dec Week 1', daysPresent: 5, daysLate: 0, performance: 'Excellent' },
    { employeeId: 5, week: 'Dec Week 1', daysPresent: 5, daysLate: 0, performance: 'Excellent' },
    { employeeId: 6, week: 'Dec Week 1', daysPresent: 4, daysLate: 0, performance: 'Good' },
    { employeeId: 7, week: 'Dec Week 1', daysPresent: 5, daysLate: 0, performance: 'Excellent' },
    // Only showing a subset of employees for the mock attendance table
];
...
export default {
    ...
    data() {
        return {
            leaveBalances: JSON.parse(JSON.stringify(mockLeaveBalances)), // Deep copy for resetting
            mockWeeklyAttendance: mockWeeklyAttendance,
            timeOffRequests: [
                { 
            ...
            // Map leave types to the balance property keys for dynamic updates
            leaveTypeMap: {
    ...
    methods: {
        getEmployeeName(employeeId) {
            // Helper to get name from employeeInfoJSON for attendance table
        ...
        submitMockRequest(type, days, employeeIndex) {
            const employeeToUse = employeeInfoJSON.employeeInformation[employeeIndex || 4]; // Default to Zanele Khumalo (ID 5)
            
            // Generate a dynamic date range based on days requested
        ...
        approveRequest(request) {
            ...
            const balanceKey = this.leaveTypeMap[request.typeOfLeave]; // Get the correct balance property name
            
            const balanceItem = this.leaveBalances.find(e => e.employeeId === request.employeeId);
            
            if (balanceItem && balanceKey) {
                // Check if enough balance is available (skip check for Unpaid Leave)
                if (request.typeOfLeave === 'Unpaid Leave' || balanceItem[balanceKey] >= daysRequested) {
                    ...
                    if (request.typeOfLeave !== 'Unpaid Leave') {
                        // Deduct from the specific accrued balance
                        balanceItem[balanceKey] -= daysRequested;
                    } 
                    // Unpaid leave is tracked (subtracted from 'unpaidBalance' in the data), but usually doesn't hit a limit.
        ...
        denyRequest(request) {
            // Only changes status; balance remains unchanged.
            request.status = 'Denied';
            alert(`Request for ${request.name} denied. Balance remains unchanged.`);
        },
        
        resetRequests() {
            // Resets working data back to the initial mock state
            ...
            // Reset balances using the initial deep copy
...
<style scoped>
/* Basic CSS for the modal backdrop (if you were to add a detailed modal here) */
.modal-backdrop {
    position: fixed;
    top: 0;
    left: 0;
